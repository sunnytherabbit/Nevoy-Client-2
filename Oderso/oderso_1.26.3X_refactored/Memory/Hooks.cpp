	...
	// Vtables
	{
		...
		// LocalPlayer::vtable
		{
			...
			if (localPlayerVtable == 0x0 || sigOffset == 0x0)
				...
				g_Hooks.Actor_isOnFireHook = std::make_unique<FuncHook>(localPlayerVtable[98], Hooks::Actor_isOnFire);
...
void* Hooks::Player_tickWorld(C_Player* _this, __int64 unk) {
	...
	if (_this == g_Data.getLocalPlayer()) {
		C_GameMode* gm = *reinterpret_cast<C_GameMode**>(reinterpret_cast<__int64>(_this) + 0x12E8);
...
bool Hooks::Actor_isOnFire(C_Entity* _this) {
	static auto oFunc = g_Hooks.Actor_isOnFireHook->GetFastcall<bool, C_Entity*>();

	if (g_Data.getLocalPlayer() != _this)
		return oFunc(_this);

	static auto noFireMod = moduleMgr->getModule<NoFire>();
	if (noFireMod != nullptr && noFireMod->isEnabled())
		return false;
